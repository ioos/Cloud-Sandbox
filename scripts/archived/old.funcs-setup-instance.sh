...
setup_spack-stack () {
  ...
  cd /save/environments
  git clone -b release/2.0 --recurse-submodules https://github.com/asascience-open/spack-stack.git spack-stack.v2.0
  cd spack-stack.v2.0
  source setup.sh
...
#  ${SPACK_STACK_DIR}/util/show_duplicate_packages.py
  ...
  SPACKOPTS="--verbose --fail-fast --cache-only"
  #spack install $SPACKOPTS 2>&1 | tee log.install
#  spack install $SPACKOPTS >& log.install
...
EOF
  ...
  sudo dnf install intel-oneapi-compiler-dpcpp-cpp-2024.2
  sudo dnf install intel-oneapi-mkl-devel-2024.2
  # sudo dnf install intel-oneapi-mkl-2024.2

  # compiler includes intel-oneapi-mpi-2021.13-2021.13.1-767

  # DON'T FORCE use of intel - use modules
  # source /opt/intel/oneapi/setvars.sh
