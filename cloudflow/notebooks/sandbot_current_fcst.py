...
def plot_roms(ds, variable, s3upload=False) -> str:
    ...
    ## VECTORS!
        ...
        # Plot the magnitude
        im = ax.contourf(da.lon_rho, da.lat_rho, mag,
             transform=ccrs.PlateCarree(), levels=12,
             cmap=cmap)
...
def plot_fvcom(ds, variable, s3upload=False) -> str:
    ...
    if variable == 'oxygen':
        ...
        return
    if variable == 'wind':
        print('wind plotting is not implemented yet')
        return
    ...
    if dims == 2:
        ...
        im = ax.tricontourf(lon, lat, nv, da[time], transform=ccrs.PlateCarree(), 
                            levels=clevs, vmin=vmin, vmax=vmax, cmap=cmap)
        
        
    if dims == 3:
        if debug: print(f"{variable} dims: {dims}")        
        im = ax.tricontourf(lon, lat, nv, da[time][siglay], transform=ccrs.PlateCarree(), 
                            levels=clevs, vmin=vmin, vmax=vmax, cmap=cmap)
...
def main():
    ...
    plot_vars = ['temp', 'zeta', 'salt', 'oxygen', 'wind']
    #plot_vars = ['temp']
    #plot_vars = ['wind']
    #plot_vars = ['zeta']
