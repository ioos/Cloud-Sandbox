...
class FVCOM_Experiment(Job):
    """ Implementation of Job class for FVCOM runs
    ...
    PTMP : str
        The base directory for the scratch disk, usually /ptmp/<username>/
    ...
    INPUTFILE : str
       auto - will generate the name based on the ofs
       path/filename - path and filename expected by the model, templated is optional 
    ...
    def parseConfig(self, cfDict):
        ...
        if self.OFS in ('nwgofs', 'negofs'):
            # correctly converts string to boolean, evaluates to True or False boolean
            self.LOCALNEST = cfDict['LOCALNEST'] == "True"
        else:
            self.LOCALNEST = None
    ...
    def make_fcstin(self):
        ...
        # Create the model namelist .in file from a template
        if OFS in ('ngofs2', 'sfbofs', 'leofs', 'lmhofs'):
