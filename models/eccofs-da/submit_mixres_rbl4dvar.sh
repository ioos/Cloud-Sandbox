...
#######################################################################
##  Entry Point - Start of script
#######################################################################


###PT Basic setup test - parameterize later
...
##---------------------------------------------------------------------
    ...
    roms_app_C=${roms_app}"6km"        # inner loop grid
     ...
     #PT ROMS_ROOT=${HOME}/ocean/repository/git/roms
     ROMS_ROOT=/save/ec2-user/roms_eccofs/roms

       HereDir=${PWD}                  # current directory

       #PT DataDir="../../Data"            # data directory 
       DataDir="/save/ec2-user/roms_eccofs/Data"            # data directory 
...
#PT        nPETsX=8                       # number PETs in the X-direction
#PT        nPETsY=8                       # number PETs in the Y-direction

#PT 192 cores single hpc8a
#PT TODO sed replace this in the template file in job setup
#PT OR pass it down from job
#PT OR compute it from NPROCS and PPN
#PT a bit of rework
...
## High-resolution grid parameters (outer loops)

         DT_da=180                     # Grid timestep (not used)

     MyNHIS_nl=60                      # NLM trajectory is saved hourly
...
#PT   MyNQCK_nl=0                       # No NLM quicksave trajectory saving
     MyNQCK_nl=30                      # NLM quicksave trajectory is saved hourly
  MyNDEFQCK_nl=0                       # No multi-file NLM trajectory

     MyNAVG_nl=720                     # time-averaged NLM trajectory is saved daily
  MyNDEFAVG_nl=0                       # No multi-file time-averaged fields
     MyNDIA_nl=720                     # time-averaged diagnostics is saved daily
  MyNDEFDIA_nl=0                       # No multi-file time-averaged diagnostic fields
 
     MyNTLM_nl=60                      # TLM trajectory is saved hourly
...
## Low-resolution grid parameters (inner loops)

         DT_da=180                     # Grid timestep (not used)

     MyNHIS_da=40                      # NLM trajectory is saved hourly
  MyNDEFHIS_da=0                       # No multi-file NLM trajectory

     MyNXTR_da=20                      # NLM decimation is saved hourly
  MyNDEFXTR_da=0                       # No multi-file NLM decimation

     MyNQCK_da=0                       # No NLM quicksave trajectory saving
#    MyNQCK_da=40                      # NLM quicksave trajectory is saved hourly
  MyNDEFQCK_da=0                       # No multi-file NLM trajectory

     MyNAVG_da=480                     # time-averaged NLM trajectory is saved daily
  MyNDEFAVG_da=0                       # No multi-file time-averaged fields
     MyNDIA_da=480                     # time-averaged diagnostics is saved daily
  MyNDEFDIA_da=0                       # No multi-file time-averaged diagnostic fields
 
     MyNTLM_da=40                      # TLM trajectory is saved hourly
     MyNADJ_da=1440                    # strong contraint, ADM saved at end
     MyNSFF_da=20                      # SFF asjustment is saved hourly
     MyNOBC_da=20                      # OBC asjustment is saved hourly

#        DT_da=240                     # Grid timestep (not used), 2x decimation

#    MyNHIS_da=30                      # NLM trajectory is saved hourly
# MyNDEFHIS_da=0                       # No multi-file NLM trajectory

#    MyNXTR_da=15                      # NLM decimation is saved hourly
# MyNDEFXTR_da=0                       # No multi-file NLM decimation

#    MyNQCK_da=40                      # NLM quicksave trajectory is saved hourly
# MyNDEFQCK_da=0                       # No multi-file NLM trajectory

#    MyNAVG_da=360                     # time-averaged NLM trajectory is saved daily
# MyNDEFAVG_da=0                       # No multi-file time-averaged fields
#    MyNDIA_da=360                     # time-averaged diagnostics is saved daily
# MyNDEFDIA_da=0                       # No multi-file time-averaged diagnostic fields
 
#    MyNTLM_da=30                      # TLM trajectory is saved hourly
#    MyNADJ_da=1080                    # strong contraint, ADM saved at end
#    MyNSFF_da=15                      # SFF asjustment is saved hourly
#    MyNOBC_da=15                      # OBC asjustment is saved hourly
...
#   ROMS_DApre="roms_da_${roms_app}_nam"           # ROMS ADM/TLM stdinp prefix, NAM
 ...
 if [[ ${restart} -eq 0 ]]; then
      ROMSiniF="eccofs3km_roms_ini_20190101.nc4"   # ROMS outer loop IC
   ROMSgeniniC="eccofs6km_roms_ini.nc4"            # ROMS Generic inner loop IC
    ROMSiniDir=${DataDir}/INI                      # ROMS IC directory
 else
       ROMSini="eccofs3km_roms_dai_20190101.nc"    # restart ROMS IC
#PT - do I need to create or fill this?
    ROMSiniDir=../2019.01.01                       # restart ROMS IC directory
 fi
...
# PT need to parameterize this for cloudflow
#set -x
#        nPETs=$(( $nPETsX * $nPETsY ))
#set +x
...
## Set configuration parameters.
  ...
  echo "    ADJ trajectory  writing: ${MyNADJ_nl}, ${MyNADJ_da} timesteps"
  echo "    AVG trajectory  writing: ${MyNAVG_nl}, ${MyNAVG_da} timesteps"
  echo "    DIA trajectory  writing: ${MyNDIA_nl}, ${MyNDIA_da} timesteps"
...
#PT might want to do this in job init since we already have that logic
  ...
  $SUBSTITUTE ${ROMS_NLinp} MyNAVG         ${MyNAVG_nl}
  $SUBSTITUTE ${ROMS_NLinp} MyNDEFAVG      ${MyNDEFAVG_nl}
  $SUBSTITUTE ${ROMS_NLinp} MyNDIA         ${MyNDIA_nl}
  $SUBSTITUTE ${ROMS_NLinp} MyNDEFDIA      ${MyNDEFDIA_nl}
  ...
  $SUBSTITUTE ${ROMS_NLinp} MyNAVG         ${MyNAVG_da}
  $SUBSTITUTE ${ROMS_NLinp} MyNDEFAVG      ${MyNDEFAVG_da}
  $SUBSTITUTE ${ROMS_NLinp} MyNDIA         ${MyNDIA_da}
  $SUBSTITUTE ${ROMS_NLinp} MyNDEFDIA      ${MyNDEFDIA_da}
...
## Set ROMS executable file links.
  ...
  if [ ${BATCH} -eq 1 ]; then
    ...
    #PT EXECUTE_A="${MPIrun} ${nPETs} ${ROMS_EXE_A} ${ROMS_NLinp}"
    #PT EXECUTE_B="${MPIrun} ${nPETs} ${ROMS_EXE_B} ${ROMS_DAinp}"

    EXECUTE_A="${MPIrun} ${HereDir}/${ROMS_EXE_A} ${ROMS_NLinp}"
    EXECUTE_B="${MPIrun} ${HereDir}/${ROMS_EXE_B} ${ROMS_DAinp}"
  fi
...
## Create ROMS 4D-Var input script 'rbl4dvar_nl.in' from template.
  ...
  if [ ${DRYRUN} -eq 0 ]; then
    ...
    if [ ${BATCH} -eq 1 ]; then
      ...
      #PT ${MPIrun} ${nPETs} ${ROMS_EXE_A} ${ROMS_NLinp} > ${nl_log}
      ${MPIrun}  ${HereDir}/${ROMS_EXE_A} ${ROMS_NLinp} > ${nl_log}
    fi
...
## Create ROMS 4D-Var input script 'rbl4dvar_da.in' from template.
    ...
    if [ ${DRYRUN} -eq 0 ]; then
      ...
      if [ ${BATCH} -eq 1 ]; then
        ...
        #PT ${MPIrun} ${nPETs} ${ROMS_EXE_B} ${ROMS_DAinp} > ${da_log}
        ${MPIrun}  ${HereDir}/${ROMS_EXE_B} ${ROMS_DAinp} > ${da_log}
      fi
